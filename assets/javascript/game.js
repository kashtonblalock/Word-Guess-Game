...
var words = [
  ...
  document
    ...
    .addEventListener("click", function newGame() {
      ...
      guessesLeft = 10;
      currentWord;
      current.textContent = "Princess:  " + underscore + "  ";
      guessed.textContent = "Letters Already Guessed: ";
      remaining.textContent = "Incorrect Guesses Remaining: 10";
      startGame();
    });
  };
  ...
  function win() {
    if (underscore.toString() === currentWordArray.toString()) {
      alert("Bippity Boppity Boop! You win!");
      totalWins++;
  ...
  function lose() {
    if (guessesLeft === 0) {
      alert("Mushu says: DISHONOR! DISHONOR ON YOUR WHOLE FAMILY! Her name was " + currentWord + ".");
    }
  }